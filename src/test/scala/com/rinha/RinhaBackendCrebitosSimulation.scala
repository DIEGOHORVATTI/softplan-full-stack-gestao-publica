...
class RinhaBackendCrebitosSimulation extends Simulation {
  ...
  def randomTipoTransacao() = Seq("r", "d", "d")(Random.between(0, 2 + 1))
  ...
  val validarConsistenciaSaldoLimite =
    (valor: Option[String], session: Session) => {
      val saldo = valor.flatMap(s => Try(s.toInt).toOption)
      val limite = toInt(session("limite").as[String])

      (saldo, limite) match {
        case (Some(s), Some(l)) if s.toInt < l.toInt * -1 =>
          Failure("Limite ultrapassado!")
        case (Some(s), Some(l)) if s.toInt >= l.toInt * -1 =>
          Success(Option("ok"))
        case _ => Failure("WTF?!")
      }
  ...
  val debitos = scenario("débitos")
    .exec { s =>
      ...
      val payload =
        s"""{"valor": ${valor}, "tipo": "d", "descricao":"${descricao}"}"""
      val session = s.setAll(
        Map(
          "descricao" -> descricao,
    ...
    .exec(
      http("débitos")
        ...
        .check(status.in(200, 422), status.saveAs("httpStatus"))
        .checkIf(s => s("httpStatus").as[String] == "200") {
          jmesPath("limite").saveAs("limite")
        }
        .checkIf(s => s("httpStatus").as[String] == "200") {
          jmesPath("saldo").validate(
            "ConsistenciaSaldoLimite - Transação",
            validarConsistenciaSaldoLimite
          )
        }
    )

  val creditos = scenario("recebíveis")
    .exec { s =>
      val descricao = randomDescricao()
      val cliente_id = randomClienteId()
      val valor = randomValorTransacao()
      val payload = s"""{"valor": ${valor}, "tipo": "r", "descricao": "${descricao}"}"""
      val session = s.setAll(
        Map(
          "descricao" -> descricao,
          "cliente_id" -> cliente_id,
    ...
    .exec(
      http("extratos")
        ...
        .check(
          ...
          jmesPath("saldo.total").validate(
            "ConsistenciaSaldoLimite - Extrato",
            validarConsistenciaSaldoLimite
          )
        )
    )
    ...
    scenario(s"validação concorrência transações - ${tipo}")
      .exec(
        http("validações")
          ...
          .body(
            StringBody(
              s"""{"valor": 1, "tipo": "${tipo}", "descricao": "validacao"}"""
            )
          )
          .check(status.is(200))
      )
  ...
  val criteriosClientes = scenario("validações")
    ...
    .exec(
      http("validações")
        ...
        .header("content-type", "application/json")
        .body(StringBody(s"""{"valor": 1, "tipo": "r", "descricao":"toma"}"""))
        .check(
          status.in(200),
          jmesPath("limite").saveAs("limite"),
          jmesPath("saldo").validate(
    ...
    .exec(
      http("validações")
        ...
        .header("content-type", "application/json")
        .body(
          StringBody(s"""{"valor": 1, "tipo": "d", "descricao": "devolve"}""")
        )
        .check(
          status.in(200),
          jmesPath("limite").saveAs("limite"),
    ...
    .exec(
      http("validações")
        .get("/clientes/#{id}/extrato")
        .check(
          jmesPath("ultimas_transacoes[0].descricao")
            .ofType[String]
            .is("devolve"),
          ...
          jmesPath("ultimas_transacoes[1].tipo").ofType[String].is("r"),
    ...
    .exec(
      http("validações")
        ...
        .header("content-type", "application/json")
        .body(
          StringBody(s"""{"valor": 1, "tipo": "r", "descricao": "danada"}""")
        )
        .check(
          status.in(200),
          ...
          http("validações")
            ...
            .check(
              jmesPath("ultimas_transacoes[0].descricao")
                .ofType[String]
                .is("danada"),
              jmesPath("ultimas_transacoes[0].tipo").ofType[String].is("r"),
              jmesPath("ultimas_transacoes[0].valor").ofType[Int].is("1"),
              jmesPath("saldo.limite").ofType[String].is("#{limite}"),
    ...
    .exec(
      http("validações")
        ...
        .header("content-type", "application/json")
        .body(
          StringBody(s"""{"valor": 1.2, "tipo": "d", "descricao": "devolve"}""")
        )
    ...
    .exec(
      http("validações")
        ...
        .header("content-type", "application/json")
        .body(
          StringBody(s"""{"valor": 1, "tipo": "x", "descricao": "devolve"}""")
        )
        .check(status.in(422, 400))
    )
    .exec(
      http("validações")
        ...
        .body(
          StringBody(
            s"""{"valor": 1, "tipo": "r", "descricao": "123456789 e mais um pouco"}"""
          )
        )
        .check(status.in(422, 400))
    )
    .exec(
      http("validações")
        .post("/clientes/#{id}/transacoes")
        .header("content-type", "application/json")
        .body(StringBody(s"""{"valor": 1, "tipo": "r", "descricao": ""}"""))
        .check(status.in(422, 400))
    )
    .exec(
      http("validações")
        .post("/clientes/#{id}/transacoes")
        .header("content-type", "application/json")
        .body(StringBody(s"""{"valor": 1, "tipo": "r", "descricao": null}"""))
        .check(status.in(422, 400))
    )

  setUp(
    validacaoTransacoesConcorrentes("d")
      .inject(
        atOnceUsers(validacaConcorrentesNumRequests)
      )
      .andThen(
        validacaoTransacoesConcorrentesSaldo(
          validacaConcorrentesNumRequests
            * -1
        ).inject(
          atOnceUsers(1)
        )
      ...
      .andThen(
        validacaoTransacoesConcorrentes("r")
          .inject(
            atOnceUsers(validacaConcorrentesNumRequests)
          )
          .andThen(
